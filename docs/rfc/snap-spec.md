...
| After | The layout `ez init` makes | done | `main.bend` moves to the top of the repository as the interface (`run`, `exec`, `start`, `par`, `code`, `text`, `ok`, `cmd`, `line`); the modules move to `src/` (`effect.bend` with the C and JS effects beside it, `argv.bend`, `answer.bend`, `file.bend`, `par.bend`); LAWS.bend and PROOF.bend sit beside `main.bend`, as in ezjson and shake. No claim changed: statements about the interface still name it as `Snap`, and statements about internals name their module (`Argv.run.plan` for `Snap.run.plan`). The paths in this RFC above are the paths of the time. |
